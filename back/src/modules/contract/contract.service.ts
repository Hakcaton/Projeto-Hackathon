...
export class ContractService {
  ...
  async generatePendingDocuments(contractId: string): Promise<void> {
    ...
    const latestRequestedDocuments: Document[] =
  ...
  async generateEmployeesPendingDocuments(contractId: string): Promise<void> {
    ...
    employees.forEach((employee) => {
      const employeeDocuments: Document[] = documents.filter(
        (doc) => doc.employee_id == employee.id,
      );
      formFields.forEach((formField) => {
        ...
        // Se já tiver documento gerado, gera apenas os subsequentes.
          ...
          if (formField.recurrence == eDocumentRecurrence.annually) {
            let nextRequestDate: Date = new Date(
              addYears(latestDocument.request_date),
            );
            while (nextRequestDate <= today) {
              let newDocument: any = {
                status: 0,
            ...
            let nextRequestDate: Date = new Date(
              addMonths(latestDocument.request_date),
            );
            while (nextRequestDate <= today) {
              let newDocument: any = {
                status: 0,
            ...
            let nextRequestDate: Date = new Date(
              addWeeks(latestDocument.request_date),
            );
            while (nextRequestDate <= today) {
              let newDocument: any = {
                status: 0,
            ...
            let nextRequestDate: Date = new Date(
              addDays(latestDocument.request_date),
            );
            while (nextRequestDate <= today) {
              let newDocument: any = {
                status: 0,
  ...
  async getPendingDocuments(contractId: string): Promise<DocumentDto[]> {
    ...
    documents.forEach((rawDoc) => {
      const formField: FormField = formFields.find(
        (field) => field.id == rawDoc.form_field_id,
      );
  ...
  async getEmployeesPendingDocuments(
    ...
    const rawDocuments = await getConnection().query(
      SELECT_EMPLOYEES_PENDING_DOCUMENTS_FROM_CONTRACT,
      [contractId],
    );
    ...
    employees.forEach((employee) => {
      const newEmployeePendingDoc: EmployeeDocumentDto =
